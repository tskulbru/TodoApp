...
public class TaskListActivity extends FragmentActivity implements
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		menu.add(Menu.NONE, 1, Menu.NONE, "Legg til");
		return true;
	}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case 1:
			// Show the fragment if its on a tablet, otherwise show a new activity
			if (mTwoPane) {
				TaskAddFragment fragment = new TaskAddFragment();
				getSupportFragmentManager().beginTransaction()
						.replace(R.id.task_add_container, fragment).commit();

			} else {
				Intent detailIntent = new Intent(this, TaskAddActivity.class);
